...
template = require './modules/templ.js'; r = template.render
tools = require './modules/tools.js'
l = require './modules/log.js'
# game controller
gameController = require './modules/game.js'
...
io.sockets.on 'connection', (client) ->
	...
	client.on 'init', ->
		refresh client
		client 'growl', "Willkommen! "+clientCount+ " Spieler verbunden.", 0
	...
	client.on 'joinGame', (gameId) ->
		await gameController.joinGame gameId, defer()
		# update footer
		updateFooterWikiwayTitle client, (data) ->
			client.emit 'updateFooterWikiwayTitle', data
		# load first article
	...
	client.on 'next', (articleId) ->
		...
		if win
			...
			r 'win', {history: history}, (data) ->
				client.emit 'updateContent', data
			r 'lose', {history: history, winner: username}, ->
...
#functions used in other controllers
